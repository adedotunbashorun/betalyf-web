    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <button class="btn btn-info m-b-10 m-l-5 pull pull-right" data-toggle="modal" data-target="#new-category" title="Add"><i class="i"></i> Add {{ $page_name }}</button>
                </h4>
                <div class="table-responsive m-t-40">
                    ...
                    @else 
                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered category" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Name</th>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                <input type="hidden" id="category_id{{$index}}" value="{{$category->id}}">
                                                <li><a href="javascript:;" id="edit{{$index}}"><i class="icon-note"></i>Edit</a></li>
                                                <li><a data-href="{{ URL::route('categories.destroy',$category->id)}}" id="btn_category_delete{{$index}}"><i class="fa fa-trash"></i>Delete</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
...
@section('modals')
    @include('admin.hospital_categories.modals._new_hospital_category')
    @include('admin.hospital_categories.modals._edit_hospital_category')
@endsection
@endsection
@section('extra_script')
    <script>
        var TOKEN = "{{csrf_token()}}";
        var UPDATE_URL = "{{URL::route('categories.update')}}";
        var GET_EDIT_INFO = "{{URL::route('categories.editInfo')}}";
        var ADD_CATEGORY = "{{URL::route('categories.store')}}";
    </script>
    <script src="{{ asset('js/pages/hospitalcategories.js') }}" type="text/javascript"></script>
@endsection
