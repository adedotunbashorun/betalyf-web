    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <button class="btn btn-info m-b-10 m-l-5 pull pull-right" data-toggle="modal" data-target="#new-information" title="Add"><i class="i"></i> Add information</button>
                </h4>
                <div class="table-responsive m-t-40">
                    @if(count($informations) < 1)
                        <div class="danger-alert">
                            <i class="fa fa-warning"></i> <em>There are no information available currently. Click on the button above to add information.</em>
                        </div>
                    @else 
                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered information" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Subject</th>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                <input type="hidden" id="information_id{{$index}}" value="{{$information->id}}">
                                                <li><a href="javascript:;" id="edit{{$index}}"><i class="icon-note"></i>Edit</a></li>
                                                <li><a data-href="{{ URL::route('informations.destroy',$information->id)}}" id="btn_information_delete{{$index}}"><i class="fa fa-trash"></i>Delete</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
...
@section('modals')
    @include('admin.information.modals._new_informations')
    @include('admin.information.modals._edit_informations')
@endsection
@endsection
@section('extra_script')
    <script>
        var TOKEN = "{{csrf_token()}}";
        var UPDATE_URL = "{{URL::route('informations.update')}}";
        var GET_EDIT_INFO = "{{URL::route('informations.editInfo')}}";
        var ADD = "{{URL::route('informations.store')}}";
    </script>
    <script src="{{ asset('js/pages/information.js') }}" type="text/javascript"></script>
@endsection
